(() => {
  ...
  let captchaSubmitAttemptCount = 0;
  let captchaSubmitExhausted = false;
  const CAPTCHA_DECODE_MESSAGE_TIMEOUT_MS = 18000;
  const CAPTCHA_MAX_AUTO_ATTEMPTS = 3;
  const CAPTCHA_MAX_SUBMIT_ATTEMPTS = 3;
  const CAPTCHA_RETRY_DELAYS_MS = [1500, 3500, 7000];
  const CAPTCHA_IMAGE_LOAD_TIMEOUT_MS = 5000;
  const CAPTCHA_IMAGE_RETRY_LIMIT = 1;
  const CAPTCHA_PAGE_RELOAD_LIMIT = 2;
  const CAPTCHA_PAGE_RELOAD_SESSION_KEY = 'hmh_captcha_page_reload_count_v1';
  ...
  function captchaImageReady(img) {
    return Boolean(img && img.complete && img.naturalWidth > 0 && img.naturalHeight > 0);
  }

  async function waitForImageReady(img, timeoutMs = CAPTCHA_IMAGE_LOAD_TIMEOUT_MS) {
    if (captchaImageReady(img)) return true;
    ...
    await new Promise((resolve, reject) => {
      ...
      const onLoad = () => {
        if (captchaImageReady(img)) finish();
        else finish(new Error('captcha-image-empty'));
      };
    ...
    if (!captchaImageReady(img)) throw new Error('captcha-image-empty');
    return true;
  }

  function getCaptchaPageReloadCount() {
    try {
      return Math.max(0, Number(sessionStorage.getItem(CAPTCHA_PAGE_RELOAD_SESSION_KEY) || 0));
    } catch (_) {
      return 0;
    }
  }

  function setCaptchaPageReloadCount(value) {
    try {
      sessionStorage.setItem(CAPTCHA_PAGE_RELOAD_SESSION_KEY, String(Math.max(0, Number(value) || 0)));
    } catch (_) {}
  }

  function resetCaptchaPageReloadCount() {
    try {
      sessionStorage.removeItem(CAPTCHA_PAGE_RELOAD_SESSION_KEY);
    } catch (_) {}
  }

  function refreshCaptchaImageRequest(img) {
    if (!img) throw new Error('captcha-image-not-found');

    // Use a real <img> navigation instead of fetch(). This deliberately lets
    // Chrome build the same request class as Haddan's own image load:
    // cookies/Referer/Sec-Fetch-* and image Accept are generated by the browser.
    // A cache-buster guarantees a network GET even if the previous image failed.
    const source = img.getAttribute('src') || img.src || '/inner/img/bc.php';
    const url = new URL(source, location.href);
    url.searchParams.set('_hmh_retry', `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    img.referrerPolicy = 'strict-origin-when-cross-origin';
    img.src = url.toString();
  }

  async function ensureCaptchaImageReady() {
    const img = document.querySelector('img[src*="bc.php"]');
    if (!img) throw new Error('captcha-image-not-found');

    if (captchaImageReady(img)) {
      resetCaptchaPageReloadCount();
      return img;
    }
    ...
    try {
      await setCaptchaStatus(
        'image-wait',
        'CAPTCHA: жду загрузку изображения…',
        { progress: 6, detail: `Жду изображение до ${Math.round(CAPTCHA_IMAGE_LOAD_TIMEOUT_MS / 1000)} секунд.` }
      );
    ...
    for (let retry = 1; retry <= CAPTCHA_IMAGE_RETRY_LIMIT; retry += 1) {
      await setCaptchaStatus(
        'image-retry',
        `CAPTCHA: изображение не загрузилось · повтор ${retry}/${CAPTCHA_IMAGE_RETRY_LIMIT}…`,
        { progress: 8, detail: 'Повторно запрашиваю штатный /inner/img/bc.php как изображение с browser-managed cookies/headers.' }
      );

      refreshCaptchaImageRequest(img);
      try {
        await waitForImageReady(img);
        resetCaptchaPageReloadCount();
        return img;
    ...
    const pageReloadCount = getCaptchaPageReloadCount();
    if (pageReloadCount < CAPTCHA_PAGE_RELOAD_LIMIT) {
      const nextCount = pageReloadCount + 1;
      setCaptchaPageReloadCount(nextCount);
      await setCaptchaStatus(
        'page-reload',
        `CAPTCHA: картинка не загрузилась · обновляю страницу ${nextCount}/${CAPTCHA_PAGE_RELOAD_LIMIT}`,
        { progress: 4, detail: 'Повтор изображения не помог; перезагружаю всю вкладку Haddan с ограничением от зацикливания.' }
      );

      const reloadRequested = await requestFullTabReload('captcha-image-unavailable');
      if (reloadRequested) throw new Error('captcha-page-reload-requested');
      throw new Error('captcha-page-reload-failed');
    }

    throw new Error('captcha-image-unavailable-after-reloads');
  }

  async function captureCaptchaImageDataUrl() {
    const img = await ensureCaptchaImageReady();
...
function captchaExpectedValue(siteRunes) {
  return (Array.isArray(siteRunes) ? siteRunes : []).map((value) => String(value)).join('');
}

function clearCaptchaAnswer() {
  const stateInput = document.querySelector('#cap_symbols');
  const symbols = document.querySelector('#wutface');
  if (!stateInput) return false;

  stateInput.value = '';
  if (symbols) {
    while (symbols.firstChild) symbols.removeChild(symbols.lastChild);
  }
...
function findBattleSubmitButton(form) {
  if (!form) return null;

  const candidates = [...form.querySelectorAll('button, input[type="submit"], input[type="button"]')]
    .filter((el) => visible(el) && enabled(el));

  const battle = candidates.find((el) => /^в\s*бой$/i.test(normalize(el.textContent || el.value || '')));
  if (battle) return battle;

  return candidates.find((el) => {
    const tag = String(el.tagName || '').toLowerCase();
    const type = String(el.getAttribute?.('type') || (tag === 'button' ? 'submit' : '')).toLowerCase();
    return type === 'submit' || (tag === 'button' && !el.hasAttribute('type'));
  }) || null;
}

function submitBattleForm(expectedValue = '') {
  ...
  const stateInput = document.querySelector('#cap_symbols');
  if (expectedValue && stateInput && String(stateInput.value || '') !== expectedValue) {
    console.warn('[Haddan Market Helper] CAPTCHA submit blocked: input mismatch', {
      expected: expectedValue,
      actual: String(stateInput.value || '')
    });
    return false;
  }

  const submitButton = findBattleSubmitButton(form);
  if (!submitButton) {
    console.error('Battle submit button not found');
    return false;
  }

  debugLog('submitButton:', submitButton);
  debugLog('text:', normalize(submitButton?.textContent || submitButton?.value));
  debugLog('type:', submitButton?.type);

  let submitEventSeen = false;
  const onSubmit = () => {
    submitEventSeen = true;
    debugLog('[Haddan Market Helper] CAPTCHA form submit event fired');
  };
  form.addEventListener('submit', onSubmit, { capture: true, once: true });

  try {
    // Important: use the same activation path as a manual user click. requestSubmit()
    // does not dispatch the button's click handlers and was intermittently ignored
    // by Haddan's page-side logic.
    submitButton.click();
    captchaSubmitAttemptCount += 1;
    debugLog('[Haddan Market Helper] CAPTCHA «В бой» click dispatched', {
      submitAttempt: captchaSubmitAttemptCount,
      submitEventSeen
    });
    return true;
  } catch (error) {
    console.warn('[Haddan Market Helper] CAPTCHA «В бой» click failed', error);
    return false;
  } finally {
...
async function ensureCaptchaAnswerApplied(siteRunes) {
  const stateInput = document.querySelector('#cap_symbols');
  if (!stateInput) return { ok: false, expectedValue: '', reused: false };

  const expectedValue = captchaExpectedValue(siteRunes);
  if (!expectedValue) return { ok: false, expectedValue, reused: false };

  // If a previous submit was ignored by Haddan, do not append the same runes a
  // second time (e.g. 24 -> 2424). Reuse the already-correct answer and only
  // retry the native «В бой» click.
  if (String(stateInput.value || '') === expectedValue) {
    await setCaptchaStatus(
      'verifying',
      'CAPTCHA: ответ уже введён, повторный ввод не нужен',
      { progress: 90, current: siteRunes.length, total: siteRunes.length, runeCount: siteRunes.length }
    );
    return { ok: true, expectedValue, reused: true };
  }

  // A partial/stale answer must be cleared before applying the decoded sequence.
  if (String(stateInput.value || '') || document.querySelector('#wutface')?.childNodes?.length) {
    clearCaptchaAnswer();
  }
  ...
  for (const value of siteRunes) {
    ...
    if (!rune) {
      ...
      return { ok: false, expectedValue, reused: false };
    }
    runes.push(rune);
  }
  ...
  const applied = await applyRunes(runes);
  debugLog('applyRunes result:', applied);
  if (!applied) return { ok: false, expectedValue, reused: false };

  const matches = String(stateInput.value || '') === expectedValue;
  if (!matches) {
    console.warn('[Haddan Market Helper] CAPTCHA input verification failed', {
      expected: expectedValue,
      actual: String(stateInput.value || '')
    });
  }

  return { ok: matches, expectedValue, reused: false };
}

async function applyCaptchaResultToCurrentPage(siteRunes, { retry = false } = {}) {
  debugLog('siteRunes:', siteRunes);

  const answer = await ensureCaptchaAnswerApplied(siteRunes);
  if (!answer.ok) return false;

  const nextSubmitAttempt = captchaSubmitAttemptCount + 1;
  await setCaptchaStatus(
    'submitting',
    retry
      ? `CAPTCHA: повторно нажимаю «В бой» · ${nextSubmitAttempt}/${CAPTCHA_MAX_SUBMIT_ATTEMPTS}`
      : 'CAPTCHA: ответ сформирован, нажимаю «В бой»…',
    {
      progress: 94,
      current: siteRunes.length,
      total: siteRunes.length,
      runeCount: siteRunes.length,
      detail: answer.reused
        ? 'Руны уже находятся в #cap_symbols; повторный ввод пропущен, выполняется только реальный click по «В бой».'
        : 'Ответ проверен в #cap_symbols; выполняется реальный click по кнопке «В бой».'
    }
  );
  await delay(retry ? 250 : 500);

  return submitBattleForm(answer.expectedValue);
}

async function retryCaptchaSubmission() {
  if (!lastCaptchaDecode?.fingerprint || !Array.isArray(lastCaptchaDecode.siteRunes) || !lastCaptchaDecode.siteRunes.length) {
    return { ok: false, stage: 'missing-decoded-answer' };
  }

  const currentImage = await captureCaptchaImageDataUrl();
  if (hashString(currentImage) !== lastCaptchaDecode.fingerprint) {
    resetCaptchaRequestState();
    lastCaptchaDecode = null;
    return { ok: false, stage: 'challenge-changed' };
  }

  const submitted = await applyCaptchaResultToCurrentPage(lastCaptchaDecode.siteRunes, { retry: true });
  if (!submitted) return { ok: false, stage: 'apply-failed' };

  lastCaptchaDecode.applied = true;
  lastCaptchaDecode.submittedAt = Date.now();
  lastCaptchaDecode.submitAttempts = captchaSubmitAttemptCount;
  return { ok: true, stage: 'resubmitted' };
}
  ...
  async function captchaIntegrationHook() {
    ...
    try {
      ...
      lastCaptchaDecode = {
        ...
        runeCount: siteRunes.length,
        siteRunes: [...siteRunes],
        expectedValue: captchaExpectedValue(siteRunes),
        applied: false,
        submitAttempts: 0
      };
      ...
      if (reason === 'captcha-page-reload-requested') {
        return { ok: false, stage: 'page-reload' };
      }
      if (reason === 'captcha-image-unavailable-after-reloads') {
        captchaRetryAt = Number.POSITIVE_INFINITY;
        await setCaptchaStatus(
          'error',
          'CAPTCHA: изображение не загрузилось после повторов',
          { progress: 0, detail: `Пробовал повторно запросить картинку и перезагрузить страницу ${CAPTCHA_PAGE_RELOAD_LIMIT} раз(а). Автоматика остановлена для этой CAPTCHA.` }
        );
        return { ok: false, stage: 'image-error', error: reason };
      }
  ...
  async function scanAutomation() {
    ...
    if (captchaHere) {
      ...
      // A button click is not proof that Haddan actually accepted the CAPTCHA.
      // If the exact same challenge remains visible, first reuse the already entered
      // #cap_symbols value and retry only the real «В бой» click. Never append the
      // same runes again (24 -> 2424). Decode/API retries are a separate budget.
      if (lastCaptchaDecode?.applied && lastCaptchaDecode.submittedAt &&
        ...
        if (captchaSubmitAttemptCount < CAPTCHA_MAX_SUBMIT_ATTEMPTS) {
          try {
            const retryResult = await retryCaptchaSubmission();
            if (retryResult.stage === 'challenge-changed') {
              captchaRetryAt = now;
            } else if (retryResult.stage === 'apply-failed') {
              lastCaptchaDecode.applied = false;
              captchaRetryAt = now;
            }
          } catch (error) {
            const reason = String(error?.message || error || 'captcha-submit-retry-failed');
            if (reason === 'captcha-page-reload-requested') {
              // The tab is about to be replaced. Preserve the explicit reload status
              // instead of letting the rest of this scan overwrite it with «жду».
              return;
            }
            console.warn('[Haddan Market Helper] CAPTCHA submit retry failed:', reason);
            lastCaptchaDecode.applied = false;
            captchaRetryAt = now;
          }
        } else {
      ...
      if (!settings.solveCaptcha) {
        ...
        await setCaptchaStatus(
          'error',
          `CAPTCHA: «В бой» нажата ${captchaSubmitAttemptCount} раз(а), но перехода нет`,
      ...
      } else if (captchaHookResult?.stage === 'error' || captchaHookResult?.stage === 'image-error') {
        // captchaIntegrationHook already stored a structured error status.
      } else if (captchaHookResult?.stage === 'page-reload') {
        // Full-tab reload was requested; do not overwrite the recovery status.
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    ...
    if (changes[AUTOMATION_KEY]?.newValue) {
      ...
      if (!wasRunning && settings.running) resetCaptchaPageReloadCount();
    }
